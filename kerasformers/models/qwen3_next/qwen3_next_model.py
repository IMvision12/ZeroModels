...
def qwen3_next_rope_tables(position_ids, rotary_dim, rope_theta):
    # Partial-rotary cos/sin over the first ``rotary_dim`` channels, in float32 to
    # match the eager forward (the layer casts as needed).
...
def qwen3_next_backbone_features(
    input_ids,
    attention_mask,
    *,
    token_embedding,
    ...
    cos, sin = qwen3_next_rope_tables(position_ids, rotary_dim, rope_theta)
...
class Qwen3NextModel(BaseModel):
    ...
    ``last_hidden_state``; use :class:`Qwen3NextTextGenerate` for logits / text.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        mlp_only = tuple(mlp_only_layers)
        ...
        final_norm = Qwen3NextRMSNorm(eps=norm_eps, name="final_norm")
        ...
        hidden = qwen3_next_backbone_features(
            inputs["input_ids"],
    ...
    def rope_tables(self, position_ids):
        return qwen3_next_rope_tables(position_ids, self.rotary_dim, self.rope_theta)
...
class Qwen3NextTextGenerate(Qwen3NextModel, BaseGeneration):
    ...
    Fast generation comes from :class:`~kerasformers.base.BaseGeneration`, fulfilled
