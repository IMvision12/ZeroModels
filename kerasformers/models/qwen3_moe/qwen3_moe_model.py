...
def qwen3_moe_rope_tables(position_ids, head_dim, rope_theta, compute_dtype):
...
def qwen3_moe_backbone_features(
    input_ids,
    attention_mask,
    *,
    token_embedding,
    ...
    cos, sin = qwen3_moe_rope_tables(position_ids, head_dim, rope_theta, compute_dtype)
...
class Qwen3MoeModel(BaseModel):
    ...
    rest are dense SwiGLU. A functional model; returns ``last_hidden_state``;
    use :class:`Qwen3MoeTextGenerate` for logits.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        final_norm = Qwen3MoeRMSNorm(eps=norm_eps, name="final_norm")
        ...
        hidden = qwen3_moe_backbone_features(
            inputs["input_ids"],
...
class Qwen3MoeTextGenerate(Qwen3MoeModel, BaseGeneration):
    """Qwen3-MoE with an LM head + fast ``.generate()`` (text -> text).
