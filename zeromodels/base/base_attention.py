...
# The attention implementation active for the current build or forward. This is
# a ContextVar, not a plain module global: ``Model.from_weights`` activates a
# model's implementation only for the scope of that model's build (so a
# functional graph bakes the right branch) and each of that model's forwards /
# generation steps (so an imperative decode reads it), always restoring the
# previous value. A per-model choice therefore never leaks to another model, a
# later forward, or another thread.
_ATTN_IMPLEMENTATION = contextvars.ContextVar(
    "attn_implementation", default=DEFAULT_ATTN_IMPLEMENTATION
)
...
def use_attn_implementation(attn_implementation):
    ...
    ``None`` selects the default (``"sdpa"``). The previous value is always
    restored on exit, so a per-model implementation never leaks past its scope.
    """
    impl = attn_implementation or DEFAULT_ATTN_IMPLEMENTATION
...
def fused_attention(
    ...
    * ``"flash"`` -- :func:`keras.ops.dot_product_attention` with
    ...
    Args:
        ...
        attn_implementation: ``"sdpa"`` / ``"flash"`` / ``None`` (use the
            implementation active in the current context).

    Returns:
        ``(batch, num_heads, q_len, head_dim)``.
    """
    impl = attn_implementation or _ATTN_IMPLEMENTATION.get()
    if impl not in VALID_ATTN_IMPL:
        raise ValueError(
            f"attn_implementation must be one of {VALID_ATTN_IMPL}, got {impl!r}"
        )
