...
class WeightLoadingMixin:
    """Unified pretrained-weight loading API shared by all zeromodels models.
    ...
    A model that ships a typed :class:`~zeromodels.base.BaseConfig` sets
    ...
    .. code-block:: python
        ...
        m = OwlViTDetect.from_weights("zeromodels/owlvit-base-patch32")
    ...
    #         "Qwen3VLConditionalGenerate", module="zeromodels.models.qwen3_vl.qwen3_vl_model",
    #         match="path")
    #
        ...
        **kwargs,
        ...
        Args:
            identifier: One of two forms:
                ...
                * a zeromodels variant string (e.g. ``"resnet50_a1_in1k"``)
...
QuantizationConfig` / scheme). When set, the model is quantized weight-only:
            ...
            cache_converted: If ``True`` (opt-in), cache the fully converted
                ...
                ``$ZEROMODELS_HOME/converted`` on first load, and on later
    ...
    def load_weights_from_url(model, url, skip_mismatch=False):
        ...
        The download is delegated to :func:`~zeromodels.conversion.\
download_weights`: a Hugging Face repo is fetched through the HF cache
    ...
    def from_variant(
        ...
        **kwargs,
        ...
        ``from_weights("qwen2-0.5b")``). Pre-converted zeromodels weights are
        ...
        if load_weights:
            if cls.BASE_WEIGHT_CONFIG is None or variant not in cls.BASE_WEIGHT_CONFIG:
                raise ValueError(
                    ...
                    f"{cls.__name__}.from_weights('zeromodels/{variant}'), or pass "
            ...
            if not isinstance(entry, dict) or not entry.get("hf_id"):
                raise ValueError(
                    ...
                    f"zeromodels weights load by Hub repo id, e.g. "
    ...
    def from_hf(
        ...
        **kwargs,
        ...
        2. **Timm-style repos** (``HF_MODEL_TYPE is None``: ResNet,
           ConvNeXt, EfficientNet, …): infers the zeromodels
        ...
        Args:
            ...
            variant: For timm-style repos, override the inferred
                zeromodels variant id (e.g., for community fine-tunes
        ...
        zeromodels_kwargs = cls.config_from_hf(hf_config)
        zeromodels_kwargs.update(kwargs)
        model = cls(**zeromodels_kwargs)
        if load_weights:
            state_dict = download_hf_state_dict(hf_id)
            completed = False
            try:
    ...
    def assert_hf_model_type(cls, hf_id, hf_config):
        ...
        if actual not in expected:
            ...
            raise ValueError(
                ...
                f"has model_type={actual!r}. This zeromodels class is the "
...
class PreprocessorMixin(keras.layers.Layer):
    """Single base for every zeromodels preprocessing layer: tokenizers,
        ...
        if identifier.startswith("hf:"):
            ...
            if "/" not in repo:
                raise ValueError(
                    ...
                    f"'/'. If {repo!r} is a zeromodels release variant, drop the "
