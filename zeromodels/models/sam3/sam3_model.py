...
class SAM3Model(BaseModel):
    ...
    Preconverted Keras weights are hosted ungated at ``zeromodels/sam3`` (SAM
    license); the gated upstream ``facebook/sam3`` also converts on the fly via the
    ``hf:`` prefix.

    Construction:

    >>> SAM3Model.from_weights("zeromodels/sam3")
    >>> SAM3Model.from_weights("hf:facebook/sam3")  # gated upstream, converts in-process
    >>> SAM3Model.from_weights("zeromodels/sam3", load_weights=False)  # random init
    ...
    def tokenizer(self):
        ...
        Pulls the CLIP ``tokenizer.json`` from the ungated ``zeromodels/sam3``
        (identical to the gated ``facebook/sam3`` file, no license gate).
        """
        if self._tokenizer is None:
            from .sam3_clip_tokenizer import SAM3CLIPTokenizer

            self._tokenizer = SAM3CLIPTokenizer(hf_id="zeromodels/sam3")
...
class _SAM3Task:
    ...
    @classmethod
    def from_weights(cls, repo_id="zeromodels/sam3", load_weights=True, **kwargs):
        """Load the SAM3 checkpoint and wrap it in this task.

        There is a single SAM3 model, hosted ungated at ``zeromodels/sam3``;
