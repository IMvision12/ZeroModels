...
def qwen3_rope_tables(position_ids, head_dim, rope_theta, compute_dtype):
...
def qwen3_backbone_features(
    ...
    *,
    ...
    cos, sin = qwen3_rope_tables(position_ids, head_dim, rope_theta, compute_dtype)
...
class Qwen3Model(BaseModel):
    ...
    before rotary), bias-free qkv projections, and 1D rotary positions. A
    functional model: the forward is a static graph over ``input_ids`` /
    ``attention_mask``. Returns ``last_hidden_state``; use :class:`Qwen3TextGenerate`
    for logits / text.

        model = Qwen3Model.from_weights("zeromodels/qwen3-0.6b")
    ...
    # Qwen3TextGenerate flips this on to also emit LM-head logits from the graph.
    output_logits = False
    ...
    def __init__(
        ...
        **kwargs,
        ...
        final_norm = Qwen3RMSNorm(eps=norm_eps, name="final_norm")
        ...
        hidden = qwen3_backbone_features(
            inputs["input_ids"],
    ...
    def rope_tables(self, position_ids):
        ...
        # wires qwen3_rope_tables directly.
        return qwen3_rope_tables(
...
class Qwen3TextGenerate(Qwen3Model, BaseGeneration):
    """Qwen3 backbone + a language-model head and fast ``.generate()``.
    ...
    inherited from :class:`Qwen3Model`.

        gen = Qwen3TextGenerate.from_weights("zeromodels/qwen3-0.6b")
