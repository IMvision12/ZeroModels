...
class TableTransformerImageProcessor(BaseImageProcessor):
    ...
    Args:
        size: Aspect-preserving resize spec
            ...
            which needs a model built with a dynamic input (``image_size=None``,
            the default). A fixed ``{"height": H, "width": W}`` instead resizes to
            exactly ``H x W`` (the reference accepts either form).
    ...
    def call(self, image) -> Dict[str, keras.KerasTensor]:
        ...
        # default); a fixed {"height", "width"} size does a plain resize to that
        # size instead (the reference accepts either). A batch is zero-padded to the
        # common max size (single-image inference is exact; see
